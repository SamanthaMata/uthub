...
function filterTiendas() {
  ...
  const precioValue = document.getElementById('filter-precio')?.value || '';
  ...
  tiendas.forEach(tienda => {
    ...
    const precioMin = parseInt(tienda.getAttribute('data-precio-min')) || 0;
    const precioMax = parseInt(tienda.getAttribute('data-precio-max')) || 999;
    ...
    // Filtro de precio: ¿algún producto de la tienda cae en el rango?
    let matchesPrecio = true;
    if (precioValue === '0-50')  matchesPrecio = precioMin <= 50;
    if (precioValue === '50-100') matchesPrecio = precioMin <= 100 && precioMax >= 50;
    if (precioValue === '100+')  matchesPrecio = precioMax >= 100;
    ...
    if (matchesSearch && matchesTipo && matchesEdificio && matchesPrecio && matchesCalificacion) {
 ...
 * AGREGAR AL CARRITO
 */
function addToCart(nombre, precio, id, btn) {
  ...
  if (existingItem) {
    ...
    cart.push({
      ...
      precio: precio,
      cantidad: 1,
      tienda: window._currentTiendaNombre || 'Tienda UThub'
    });
  }
...
function procesarPedido() {
  ...
  setTimeout(() => {
    ...
    // Disparar notificación al sistema
    if (window.UThubNotifications) {
      UThubNotifications.addNotification({
        tipo: 'pedido_estado',
        icono: '✅',
        titulo: '¡Pedido confirmado!',
        mensaje: 'Tu pedido #' + String(nuevoPedido.id).slice(-5) + ' fue recibido correctamente.',
